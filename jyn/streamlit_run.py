# https://s0cqcxuy3p.feishu.cn/wiki/VNBAw14ecituHhkj4d0cdNpSngf
# 事件驱动择时研究V231225.py
# 本地运行：
#           streamlit run jyn/streamlit_run.py --theme.base=dark

import os
os.environ['base_path'] = r"E:\trade\trade\github\czsc\jyn\CTA研究"    # 回测结果保存路径
...
def show_trader(trader: CzscTrader, files):
    ...
    for freq in freqs:
        ...
        kline = KlineChart(n_rows=3, row_heights=(0.8, 0.1, 0.1), title='', width="100%", height=1000)
...
def main():
    with st.sidebar:
        selected = option_menu("事件驱动择时app", ["策略回放", '策略回测'],
